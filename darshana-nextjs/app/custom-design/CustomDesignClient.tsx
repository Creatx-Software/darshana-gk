...
import type { CustomDesignItem, CustomDesignPage } from '@/lib/api/strapi';
import { API_URL } from '@/lib/api/config';

interface CustomDesignClientProps {
  designItems: CustomDesignItem[];
  pageSettings: CustomDesignPage | null;
}
...
export default function CustomDesignClient({ designItems, pageSettings }: CustomDesignClientProps) {
  ...
  const nextImage = () => {
    if (selectedImage !== null && designItems.length > 0) {
      setSelectedImage((selectedImage + 1) % designItems.length);
    }
  };
  ...
  const prevImage = () => {
    if (selectedImage !== null && designItems.length > 0) {
      setSelectedImage(
        selectedImage === 0 ? designItems.length - 1 : selectedImage - 1
      );
    }
  };
  ...
  const catalogFileUrl = getFileUrl(pageSettings?.catalogFile);
  ...
  return (
    <>
        ...
        <div className="container">
          <div className="catalog-box" data-aos="zoom-in">
            <div className="catalog-content">
              <h2>{pageSettings?.catalogTitle || 'Download Our Design Catalog'}</h2>
              <p>
                {pageSettings?.catalogDescription || 'Explore our collection of past custom projects and get inspired for your own design. Our catalog showcases the range of possibilities available with custom stone carving.'}
              </p>
              {catalogFileUrl ? (
                <a href={catalogFileUrl} target="_blank" rel="noopener noreferrer" className="btn btn-stone catalog-btn">
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                ...
                <button className="btn btn-stone catalog-btn" disabled style={{ opacity: 0.5 }}>
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                  ...
                  >
                    <div className="gallery-image">
                      {imageUrl ? (
                        ...
                        <div style={{ width: 800, height: 600, background: '#333' }} />
                      )}
      ...
      >
        ...
        {selectedImage !== null && designItems[selectedImage] && (
          <img
            className="lightbox-image"
            src={getImageUrl(designItems[selectedImage].image) || ''}
            alt={designItems[selectedImage].title}
          />
        )}
      </div>
    </>
